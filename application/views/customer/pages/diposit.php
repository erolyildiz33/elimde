                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-8">
                                    <select class="form-control basic-single" name="method" required onchange="Fee()" id="payment_method" disabled>
                                        ...
                                        <?php foreach ($payment_gateway as $key => $value) {   if ($value->agent!='Blockchain') { ?>
...
<script type="text/javascript">
 ...
 var level = document.forms['deposit_form'].elements['level'].value;
        ...
        console.log(amount + " :: " + method + "::" + 'deposit' + "::" + csrf_test_name);
		...
		$.ajax({
                ...
                'data': {'method': method,'level':'deposit','amount':amount,'csrf_test_name':csrf_test_name },
                'dataType': "JSON",
                'success': function(data) { 
                    if(data){
                        $('[name="amount"]').val(data.amount);
                        $('[name="fees"]').val(data.fees);
                        $('#fee').text("Fees is "+data.fees+" Need Balans to:"+data.amount);                    
                    } else {
                        alert('Error!');
                    }  
                }
