                ...
                <?php echo form_hidden('id', $payment_gateway->id) ?> 
                    <div class="form-group row">
                        ...
                        if ($payment_gateway->identity=='bitcoin') {
                            ...
                            echo form_hidden('secret_key', $payment_gateway->secret_key);
                        }
                        ...
                        if ($payment_gateway->identity=='blockchain') {

                            $level1 = display('Host');
                            $level2 = display('private_key');
                            $level3 = display('secret_key');
                        }
                        else {
                            $level1 = display('public_key');
                            $level2 = display('private_key');
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-6">
                            <input name="public_key" value="<?php echo $payment_gateway->public_key ?>" class="form-control" type="text" id="host">
                        </div>
                    </div> 
                    <div class="form-group row">
                        <label for="private_key" class="col-sm-4 col-form-label"><?php echo $level2; ?> <i class="text-danger">*</i></label>
                        <div class="col-sm-6">
                            <input name="private_key" value="<?php echo $payment_gateway->private_key ?>" class="form-control" type="text" id="public_key">
                        </div>
                    </div>
                    <?php if ($payment_gateway->identity=='blockchain') { ?>
                        <div class="form-group row">
                        <label for="private_key" class="col-sm-4 col-form-label"><?php echo $level3; ?> <i class="text-danger">*</i></label>
                        <div class="col-sm-6">
...
<?php if ($payment_gateway->identity=='paystack') { ?>
                ...
                <?php echo form_open_multipart("backend/payment_gateway/payment_gateway/form/$payment_gateway->id") ?>
                <?php echo form_hidden('id', $payment_gateway->id) ?> 
                <?php echo form_hidden('agent', $payment_gateway->agent) ?> 
                <?php echo form_hidden('public_key', $payment_gateway->public_key) ?> 
                <?php echo form_hidden('private_key', $payment_gateway->private_key) ?>
                    <div class="form-group row">
                        <label for="secret_key" class="col-sm-4 col-form-label">Currency Converter API</label>
                        <div class="col-sm-6">
                            <input name="secret_key" value="<?php echo $payment_gateway->secret_key ?>" class="form-control" type="text" id="secret_key">
                        </div>
                        <div class='col-sm-2'>
                           <a href='https://www.currencyconverterapi.com/' target='_blank'>Get API</a>
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-sm-9 col-sm-offset-3">
