...
class Withdraw extends CI_Controller 
{
  ...
  $rDate=$this->db->select("request_date")->from("withdraw")->where('user_id',$this->session->userdata('user_id'))->limit(1)->order_by("request_date","desc")->get()->row();
  if ($rDate){
        $date_1 = date_create(date('Y-m-d'));
        $date_2 = date_create($rDate->request_date);
...
public function withdraw_details($id=NULL)
{
    ...
    public function store()
    {
        ...
        if($this->form_validation->run()){
			...
			'g_amount'=>$this->input->post('amount')- ($this->input->post('amount')*5/100)
                    );
