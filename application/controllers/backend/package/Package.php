...
public function p_list(){
	...
	{ 
		...
		if (!empty($this->input->post('daily_roi'))){ $this->form_validation->set_rules('daily_roi', display('daily_roi'),'required|max_length[11]');}
		if (!empty($this->input->post('weekly_roi'))){ $this->form_validation->set_rules('weekly_roi', display('weekly_roi'),'required|max_length[11]');}
		...
		$this->form_validation->set_rules('status', display('p_type'),'required|max_length[1]');
		$this->form_validation->set_rules('period', display('period'),'required');
		if (empty($this->input->post('daily_roi')))
			{
				$daily_roim='0';
			} else {$daily_roim=$this->input->post('daily_roi');}
		if (empty($this->input->post('weekly_roi'))){$weekly_roim='0';} else {$weekly_roim=$this->input->post('weekly_roi');}
/*-----------------------------------*/ 
$data['package'] = (object)$userdata = array(
	...
	'daily_roi' 	  => $daily_roim,
	'weekly_roi' 	  => $weekly_roim,
