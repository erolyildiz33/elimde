...
class Home extends CI_Controller 
{
    ...
    {
       $data['title'] = display('blockday');
       $data['blockday']=$this->db->select("day")->from('blockday')->where('uid',1)->get()->row()->day;
       $data['content'] = $this->load->view('backend/block', $data, true);
       $this->load->view('backend/layout/main_wrapper', $data);

       if ($this->input->post('blockday')){


        if ($this->db->where('uid',1)->update('blockday', array('day'   => $this->input->post('blockday')))) {
            $this->toastr->success( display('save_successfully'));
        } else {
            $this->toastr->error( display('please_try_again'));
        }
        redirect('Home/block');
    }
}
...
public function coinmarket()
{
    ...
    public function buy()
    {
        ...
        if ($this->form_validation->run()) 
        {
            if (!$this->input->valid_ip($this->input->ip_address())){
                $this->toastr->error( display("ip_address")." Invalid");
                redirect("buy");
    ...
    public function sells()
    {
        ...
        if ($this->form_validation->run()) 
        {
            ...
            if ($this->sell_model->create($userdata)) {
                ...
                $this->toastr->success( display('sell_successfully'));
    ...
    public function register()
    {
        ...
        $this->form_validation->set_rules("phone", display('mobile'), "trim|required|max_length[12]|min_length[12]"); 
        ...
        if ($this->form_validation->run() === true) {
            ...
            if (!$sponsor_user_id){
                $this->toastr->error( "Valid Sponsor ID Required");
                redirect("register");
                return false;
            }
            ...
            if ($usercheck->num_rows() > 0) {
                if ($usercheck->row()->oauth_provider=='facebook' && $usercheck->row()->status==0 || $usercheck->row()->oauth_provider=='google' && $usercheck->row()->status==0) {
                    ...
                    if ($checkDuplictuser->num_rows() > 0){
                        $this->toastr->error( display('username_used'));
                        redirect("register");
                    }
                    ...
                    $this->toastr->success( display('account_create_success_social'));
                    redirect('home');

                }else {
                    $this->toastr->error( display('email_used')." ".display('username_used'));
                    redirect("register");
                ...
                if ($duplicatemail->num_rows() > 0){
                    $this->toastr->error( display('email_used'));
                    redirect("register");
                ...
                if($this->web_model->registerUser($data)){
                    ...
                    $this->toastr->success( display('account_create_active_link'));
                    redirect("home");
...
public function activeAcc($activecode='')
{
    ...
    if ($user->num_rows() > 0){
        ...
        $this->toastr->success( display('active_account'));
        redirect("home");

    }       
    else {
        $this->toastr->error( display('wrong_try_activation'));
        redirect("home");
    ...
    if ($this->session->userdata('buy')->payment_method=='bitcoin') {
            ...
            ******************************/
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL            => "https://free.currconv.com/api/v7/convert?q=USD_NGN&compact=ultra&apiKey=$gateway->secret_key",
            ...
            ******************************/
            if ($this->buy_model->create($this->session->userdata('buy'))) {
                $this->session->unset_userdata('buy');
                $this->toastr->success( display('payment_successfully'));
                redirect("buy");
...
{
    $sndemail=$this->input->post('email');
       ...
       if ($this->form_validation->run()) {
        ...
        $varify_code = $this->randomID();
            ...
            ******************************/
            ...
            $checkmail= $this->common_model->countuser($this->input->post('email'));
...
public function resetPassword()
{
    ...
    if ($this->form_validation->run()) {
        if($chkdata!=NULL) {
            ...
            $this->toastr->success(display('update_successfully'));
            redirect('home');
